...
void updateFlight(float dt)
{
	...
	// integrate position (vertical only for now)
...
void processMouseMotion(int xx, int yy)
{
	...
	if (activeCam == 2)
	{
		...
		{											   // ORBIT (LMB)
			...
			// clamp pitch only
			if (followPitchOffsetDeg > maxPitchDeg)
				followPitchOffsetDeg = maxPitchDeg;
			else if (followPitchOffsetDeg < minPitchDeg)
				followPitchOffsetDeg = minPitchDeg;
		...
		{ // ZOOM (RMB)
			...
			if (followDistance < 5.0f)
				followDistance = 5.0f;
			if (followDistance > 60.0f)
